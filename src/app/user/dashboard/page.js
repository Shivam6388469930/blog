    ...
    <div className="p-6 w-full mb-6">
      <div className="p-6 b">
        ...
        <div className="flex flex-col sm:flex-row justify-between mb-4 gap-4">
          <div>
            <h3 className="text-2xl font-bold">Blog Dashboard</h3>
            <p className="">Manage your content and analytics seamlessly</p>
          </div>
          <a href="/user/new_article">
            <button className="px-6 py-3 text-white bg-green-500 hover:bg-green-600 rounded-lg shadow-md w-full sm:w-auto transition-all">
              New Article +
            </button>
          </a>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
          {cards.map((card, index) => (
            <div
              key={index}
              className="p-6 bg-white text-gray-800 rounded-xl shadow-xl hover:scale-105 transform transition-all duration-300"
            >
              <p className="text-xl font-semibold">{card.name}</p>
              <p className="text-3xl font-bold">{card.number}</p>
              <p className="text-green-500">{card.growth}</p>
            </div>
          ))}
        </div>

        {/* Articles Table */}
        <div className="mt-8 overflow-x-auto bg-white p-4 rounded-xl shadow-xl">
          <table className="w-full border-collapse">
            <thead className="bg-gray-900 text-white">
              <tr>
                {["Title", "Status", "Comments", "Date", "Actions"].map((head) => (
                  <th key={head} className="py-4 px-6 text-left text-sm font-medium">{head}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {articles.length === 0 ? (
                ...
                articles.map((article) => (
                  <tr key={article._id} className="border-b text-sm">
                    ...
                    <td className="py-4 px-6 text-green-600">Published</td>
      ...
      {/* Error Message */}
      {error && (
        <div className="bg-red-600 text-white p-4 rounded-xl mt-4 shadow-lg">
          {error}
        </div>
      )}
          ...
          <div className="bg-white p-8 rounded-xl shadow-xl w-1/3">
            ...
            <div className="flex justify-end gap-4">
              <button
                ...
                className="px-5 py-3 bg-gray-400 text-white rounded"
              >
                Cancel
              </button>
              <button
